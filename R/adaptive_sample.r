##' @title Spatially adaptive sampling
...
adaptive.sample <- function(obj1, obj2, pred.var.col = NULL, excd.prob.col = NULL,
                            batch.size = 1, delta, criterion,
                            poly = NULL, plotit = TRUE){

  if(!identical(class(obj1), class(obj2)))
    stop("\n 'obj1' and 'obj2' must be of the same class")
  obj1.origin <- obj1;obj2.origin <- obj2
  ...
  if(inherits(poly, 'Spatial')){
    ...
    poly <- poly
    if (!identical(st_crs(obj1),st_crs(poly)))
  ...
  dsq <- delta^2
  if ((npts * pi * dsq/4 > as.numeric(sf::st_area(poly.shape))))
    stop("\n Polygon is too small to fit ", batch.size,
         "  adaptive points, in addtion to existing ",
         dim(old.design)[1], " points,",
 ...
 if (criterion == "predvar"){
    ...
    avail.locs <- xnotiny(sf::st_coordinates(obj.var.ord), old.design)
    totalbatch = 1
    counter = 1
    rejected <- NULL
    delta = delta
    ...
    while(totalbatch <= batch.size){
      distance <- pdist(adapt.sample, avail.locs[counter,])@dist
      min.dist <- min(distance)
      if(min.dist > delta){
        adapt.sample <- as.data.frame(rbind(adapt.sample, avail.locs[counter,]))
      ...
      if (counter > dim(avail.locs)[1]){
        warning("\n For the given 'delta' and 'batch.size', only ", dim(adapt.sample)[1] - dim(old.design)[1],
                " adaptive samples placed out of ", batch.size, " Consider revising 'delta' and/or 'batch.size'")
        break
      }
    }
  }
  ...
  if(criterion == "exceedprob") {
    if(is.null(excd.prob.col) | length(excd.prob.col) == 0)
      stop("\n provide exceedance probabilities")
    obj1.df <- `st_geometry<-`(obj1, NULL)
    obj.exceed.ord <- obj1[order(abs(obj1.df[,excd.prob.col] - 0.5)),]
    avail.locs <- xnotiny(sf::st_coordinates(obj.exceed.ord), old.design)
    totalbatch = 1
    counter = 1
    rejected = NULL
    delta = delta
  ...
  if(plotit==TRUE){
    curr.sample <- adapt.sample %>% as.data.frame %>% sf::st_as_sf(coords = c(1,2))
    st_crs(curr.sample) <- st_crs(poly.shape)
    par(oma=c(5.1, 5.1, 5.1, 5.1), mar=c(5.5, 5.1, 4.1, 2.1), mgp=c(3, 1, 0), las=0)
    plot(st_geometry(curr.sample),pch=19,col="red", xlab="longitude",ylab="lattitude", axes = T,
         xlim = c(range(st_coordinates(poly.shape)[,1])),
         ylim = c(range(st_coordinates(poly.shape)[,2])))
    title(main = "Existing design plus adaptive samples", font.main = 3, cex.main = 1.2, col.main = "blue")
    ...
    legend(par('usr')[2], par('usr')[4],  xpd=NA, bty='n',
           legend = c("init. samples","adapt. samples"), #"available locs",
           col = c("darkblue","red"),pch = 19, cex = 1, title = "Locations",
           text.font=4, bg='white', xjust = 0, title.adj = 0.2, title.col = 1)
  }
  ...
  res$sample.locs$curr.sample <- adapt.sample %>% as.data.frame %>% sf::st_as_sf(coords = c(1,2))
  st_crs(res$sample.locs$curr.sample) <- st_crs(obj2)
  res$sample.locs$prev.sample <- old.design %>% as.data.frame %>% sf::st_as_sf(coords = c(1,2))
  st_crs(res$sample.locs$prev.sample) <- st_crs(obj2)
  res$sample.locs$added.sample <- xnotiny(adapt.sample, old.design) %>% as.data.frame %>% sf::st_as_sf(coords = c(1,2))
